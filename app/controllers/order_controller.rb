class OrderController < ApplicationController
	...
	def create
		...
		if params[:bowl].to_f > 0
			...
			@order.contents += "bowl"
			@order.contents += ","
			@order.total += 10* params[:bowl].to_f
		end
		#check for comments
		if params[:comments]
